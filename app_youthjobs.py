...
if st.session_state["job_df"] is not None:
    ...
    with col2:
        ...
        if st.button("💾 Save to Cloud"):
            ...
            if not fid:
                st.error("❌ ERROR: Missing 'folder_id'. Check [youthjobs] in secrets.toml.")
            else:
                st.info(f"🕵️ Using Folder ID: `{fid}`")
                ...
                with st.spinner("Saving..."):
                    ...
                    if not url:
                        st.error("❌ Failed to create Spreadsheet. Check Folder Permissions (Share with y4jaiops@gmail.com).")
                    else:
                        st.success(f"✅ Sheet Located: {url}")
                        
                        # 3. APPEND DATA
                        if append_batch_to_sheet(url, edited_df.to_dict('records')):
                            st.balloons()
                            st.success("✅ Saved successfully! Resetting...")
                            time.sleep(2) 
                            full_reset()
                            st.rerun()
                        else:
                            st.error("❌ Database connection established, but failed to write rows.")
